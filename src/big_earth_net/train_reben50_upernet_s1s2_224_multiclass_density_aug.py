...
Main features:
    - reBEN ResNet50 encoder
    - partial fine-tuning: layer4 + decoder
    - UPerNet-style decoder
    - 5-class segmentation output
    - Weighted CrossEntropy + Multiclass Dice
    - stronger density-dependent augmentation
    - collapsed binary favela metrics
    - per-class precision, recall, Dice, IoU
    - density-bin binary metrics
    - final Markdown metrics report
"""
...
# ---------------------------------------------------------------------
# IMPORTANT: force non-GUI Matplotlib backend before importing pyplot.
# This prevents Windows/Tkinter crashes during long training runs.
# ---------------------------------------------------------------------
import matplotlib
...
def read_raster_chw(
    ...
    with rasterio.open(path) as src:
        band_indices = choose_band_indices(
            src.count,
            preferred_band_indices,
            expected_count,
        )
...
# ---------------------------------------------------------------------
# Density classes and augmentation
# ---------------------------------------------------------------------
...
def _split_augmentation_name(aug: str) -> Tuple[str, Optional[str]]:
    if aug == "none":
        return "none", None
...
def _apply_geometric_augmentation(
    ...
    if geom_aug == "none":
        ...
        x = np.flip(x, axis=1)
        y = np.flip(y, axis=0)
    elif geom_aug == "hflip_rot90":
        x = np.flip(x, axis=2)
        y = np.flip(y, axis=1)
        x = np.rot90(x, k=1, axes=(1, 2))
        y = np.rot90(y, k=1, axes=(0, 1))
    elif geom_aug == "vflip_rot90":
        x = np.flip(x, axis=1)
        y = np.flip(y, axis=0)
        x = np.rot90(x, k=1, axes=(1, 2))
        y = np.rot90(y, k=1, axes=(0, 1))
    else:
        raise ValueError(f"Unknown geometric augmentation: {geom_aug}")

    return x, y


def _apply_radiometric_augmentation(x: np.ndarray, photo_aug: Optional[str]) -> np.ndarray:
    if photo_aug is None:
        return x

    x = x.astype(np.float32, copy=True)

    if photo_aug == "gain_up":
        x *= 1.10
    elif photo_aug == "gain_down":
        x *= 0.90
    elif photo_aug == "s1_gain_up":
        x[:2] *= 1.10
    elif photo_aug == "s1_gain_down":
        x[:2] *= 0.90
    elif photo_aug == "s2_gain_up":
        x[2:] *= 1.10
    elif photo_aug == "s2_gain_down":
        x[2:] *= 0.90
    else:
        raise ValueError(f"Unknown radiometric augmentation: {photo_aug}")
...
def apply_augmentation(x: np.ndarray, y: np.ndarray, aug: str) -> Tuple[np.ndarray, np.ndarray]:
    geom_aug, photo_aug = _split_augmentation_name(str(aug))
    x, y = _apply_geometric_augmentation(x, y, geom_aug)
    x = _apply_radiometric_augmentation(x, photo_aug)
...
def infer_columns(df: pd.DataFrame) -> DatasetColumns:
    ...
    if s1_col is None and "sar_path" in df.columns:
        s1_col = "sar_path"
    ...
    if s2_col is None and "optical_path" in df.columns:
        s2_col = "optical_path"
...
class FavelaDensityMulticlassDataset(Dataset):
    ...
    def _get_density_class(self, row: pd.Series, binary_mask: np.ndarray) -> Tuple[float, int]:
        if "gt_pos_pct_computed" in row.index and pd.notna(row["gt_pos_pct_computed"]):
            gt_pct = float(row["gt_pos_pct_computed"])
        elif self.columns.gt_pct_col is not None and self.columns.gt_pct_col in row.index and pd.notna(row[self.columns.gt_pct_col]):
...
def compute_density_for_dataframe(
    ...
    if tqdm is not None:
        iterator = tqdm(
            iterator,
            total=len(df),
            desc="Computing density classes",
            unit="patch",
            dynamic_ncols=True,
        )
...
def build_density_augmented_train_manifest(
    ...
    if output_csv.exists():
        print(f"Loading cached augmented manifest: {output_csv}")
    ...
    print("\nRows by augmentation:")
    print(aug_df["augmentation"].value_counts().to_string())
...
def load_reben_weights_if_available(model: FavelaMulticlassSegModel, repo_id: str) -> Dict[str, object]:
    ...
    try:
        ...
        if isinstance(state, dict):
            for key in ["state_dict", "model", "model_state_dict", "net", "network", "encoder", "backbone"]:
        ...
        def find_matching_target_key(src_key: str, src_tensor: torch.Tensor) -> Optional[str]:
            cleaned = clean_key(src_key)

            if cleaned in target_state:
                tgt = target_state[cleaned]
                if tuple(src_tensor.shape) == tuple(tgt.shape):
                    return cleaned

            suffix_matches = []
            for tk in target_keys:
                if cleaned.endswith(tk):
                    tgt = target_state[tk]
                    if tuple(src_tensor.shape) == tuple(tgt.shape):
                        suffix_matches.append(tk)
        ...
        for src_key, src_tensor in state.items():
            if not torch.is_tensor(src_tensor):
                skipped += 1
                continue

            target_key = find_matching_target_key(src_key, src_tensor)

            if target_key is not None:
                load_state[target_key] = src_tensor
                matched += 1
                continue

            cleaned = clean_key(src_key)
            if cleaned.endswith("conv1.weight") and "conv1.weight" in target_state:
                tgt = target_state["conv1.weight"]

                if (
                    src_tensor.ndim == 4
                    and tgt.ndim == 4
                    and src_tensor.shape[0] == tgt.shape[0]
                    and src_tensor.shape[2:] == tgt.shape[2:]
                ):
                    new_weight = tgt.clone()
                    n_common = min(src_tensor.shape[1], tgt.shape[1])

                    new_weight[:, :n_common, :, :] = src_tensor[:, :n_common, :, :]

                    if tgt.shape[1] > src_tensor.shape[1]:
                        mean_extra = src_tensor.mean(dim=1, keepdim=True)
                        new_weight[:, n_common:, :, :] = mean_extra.repeat(
                            1,
                            tgt.shape[1] - n_common,
                            1,
                            1,
                        )

                    load_state["conv1.weight"] = new_weight
                    matched += 1
                    partial_conv1_loaded = True
                    continue
        ...
        info["partial_conv1_loaded"] = partial_conv1_loaded
        info["skipped_keys"] = skipped
        info["loaded"] = matched > 0
        info["message"] = (
            f"Loaded encoder weights from {ckpt_path}. "
            f"Matched keys: {matched}. "
            f"Partial conv1 loaded: {partial_conv1_loaded}. "
            f"Skipped keys: {skipped}. "
            f"Missing keys after load: {len(missing)}. "
        ...
        if matched == 0:
            print("\n[WARNING] No pretrained keys matched.")
            print("First 30 checkpoint keys:")
            for i, k in enumerate(list(state.keys())[:30]):
                v = state[k]
                shape = tuple(v.shape) if torch.is_tensor(v) else "not_tensor"
                print(f"  {i:02d}: {k} {shape}")

            print("\nFirst 30 target ResNet50 keys:")
            for i, k in enumerate(target_keys[:30]):
                print(f"  {i:02d}: {k} {tuple(target_state[k].shape)}")
...
def evaluate(
    ...
    for batch in iterator:
        ...
        cm_total += confusion_matrix_multiclass(
            preds.cpu(),
            targets.cpu(),
            num_classes=num_classes,
        )
...
def plot_history(history_df: pd.DataFrame, output_dir: Path) -> None:
    ...
    def plot_metric(metric: str, filename: str, ylabel: str):
        ...
        if train_col in history_df.columns:
            ax.plot(history_df["epoch"], history_df[train_col], label=f"train {metric}")

        if val_col in history_df.columns:
            ax.plot(history_df["epoch"], history_df[val_col], label=f"val {metric}")
    ...
    try:
        ...
        plot_metric("mean_favela_class_dice", "curve_mean_favela_class_dice.png", "Mean Favela-Class Dice")
    except Exception as exc:
        print(f"[WARNING] Plotting failed, but training will continue. Error: {exc}")
    finally:
        plt.close("all")
...
def write_final_metrics_report(
    ...
    val_density_df: pd.DataFrame,
    test_density_df: pd.DataFrame,
    final_summary: Dict[str, object],
) -> None:
    report_path = run_dir / "metrics" / "final_metrics_report.md"
    ...
    lines.append("# Final Metrics Report")
    lines.append("")
    lines.append(f"Run directory: `{run_dir}`")
    lines.append(f"Best epoch: `{final_summary.get('best_epoch', 'unknown')}`")
    lines.append(f"Selection metric: `{final_summary.get('selection_metric_name', 'unknown')}`")
    lines.append("")

    lines.append("## Collapsed Binary Favela Metrics")
    lines.append("")
    lines.append("These metrics collapse all favela classes into one binary favela class:")
    lines.append("")
    lines.append("`binary favela = tiny OR small OR medium OR large`")
    lines.append("")
    lines.append("| Split | IoU | Dice | Precision | Recall | Accuracy | GT favela % | Pred favela % | Pred - GT % |")
    lines.append("|---|---:|---:|---:|---:|---:|---:|---:|---:|")

    for split_name, summary in [("Validation", val_summary), ("Test", test_summary)]:
        lines.append(
            f"| {split_name} "
            f"| {fmt_float(summary['binary_iou_favela'])} "
            f"| {fmt_float(summary['binary_dice_favela'])} "
            f"| {fmt_float(summary['binary_precision_favela'])} "
            f"| {fmt_float(summary['binary_recall_favela'])} "
            f"| {fmt_float(summary['binary_accuracy'])} "
            f"| {fmt_float(summary['binary_gt_pos_pct'])} "
            f"| {fmt_float(summary['binary_pred_pos_pct'])} "
            f"| {fmt_float(summary['binary_pred_minus_gt_pct'])} |"
        )

    for title, df in [
        ("Per-Class Multiclass Metrics: Validation", val_class_df),
        ("Per-Class Multiclass Metrics: Test", test_class_df),
    ]:
        lines.append("")
        lines.append(f"## {title}")
        lines.append("")
        lines.append("| Class ID | Class | IoU | Dice | Precision | Recall | Support pixels | Predicted pixels |")
        lines.append("|---:|---|---:|---:|---:|---:|---:|---:|")
        ...
        for _, row in df.iterrows():
            lines.append(
                ...
                f"| {row['class_name']} "
                f"| {fmt_float(row['iou'])} "
                f"| {fmt_float(row['dice'])} "
                f"| {fmt_float(row['precision'])} "
                f"| {fmt_float(row['recall'])} "
                f"| {int(row['support_pixels'])} "
                f"| {int(row['pred_pixels'])} |"
            )

    for title, df in [
        ("Binary Metrics by Patch-Density Class: Validation", val_density_df),
        ("Binary Metrics by Patch-Density Class: Test", test_density_df),
    ]:
        lines.append("")
        lines.append(f"## {title}")
        lines.append("")
        lines.append("| Density class | Patches | Binary IoU | Binary Dice | Precision | Recall | GT % | Pred % | Pred - GT % |")
        lines.append("|---|---:|---:|---:|---:|---:|---:|---:|---:|")
        ...
        if not df.empty:
            for _, row in df.iterrows():
                lines.append(
                    f"| {row['density_label']} "
                    f"| {int(row['n_patches'])} "
                    f"| {fmt_float(row['binary_iou'])} "
                    f"| {fmt_float(row['binary_dice'])} "
                    f"| {fmt_float(row['binary_precision'])} "
                    f"| {fmt_float(row['binary_recall'])} "
                    f"| {fmt_float(row['binary_gt_pos_pct'])} "
                    f"| {fmt_float(row['binary_pred_pos_pct'])} "
                    f"| {fmt_float(row['binary_pred_minus_gt_pct'])} |"
                )

    lines.append("")
    lines.append("## Notes")
    lines.append("")
    lines.append("- The model is trained as a five-class multiclass segmentation model.")
    lines.append("- For binary evaluation, all non-background classes are collapsed into one favela class.")
    lines.append("- The favela classes are patch-density classes, not true object-size classes.")
    lines.append("- Therefore, `tiny`, `small`, `medium`, and `large` refer to the favela density of the patch.")

    report_path.write_text("\n".join(lines), encoding="utf-8")
    print(f"Saved final metrics report to: {report_path}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--run-name", type=str, default="exp06_reben50_layer4_multiclass_density_aug61k_wce_dice_bs64")
    ...
    parser.add_argument("--tiny-aug", type=int, default=12)
    parser.add_argument("--small-aug", type=int, default=9)
    parser.add_argument("--medium-aug", type=int, default=7)
    parser.add_argument("--large-aug", type=int, default=6)
    parser.add_argument("--max-aug-per-patch", type=int, default=16)

    parser.add_argument(
        "--selection-metric",
        type=str,
        default="combined",
        choices=["binary_iou", "mean_favela_class_iou", "combined"],
    )
...
def main() -> None:
    ...
    save_json(summary_initial, run_dir / "summary_initial.json")

    history_rows = []

    history_csv = run_dir / "history.csv"
    if args.resume != "none" and history_csv.exists():
        old_history = pd.read_csv(history_csv)
        history_rows = old_history.to_dict(orient="records")
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if args.selection_metric == "binary_iou":
            selection_metric = val_summary["binary_iou_favela"]
        elif args.selection_metric == "mean_favela_class_iou":
            selection_metric = val_summary["mean_favela_class_iou"]
        elif args.selection_metric == "combined":
            selection_metric = (
                0.5 * val_summary["binary_iou_favela"]
                + 0.5 * val_summary["mean_favela_class_iou"]
            )
        else:
        ...
        if selection_metric > best_metric:
            ...
            print(
                ...
                f"selection_metric={best_metric:.4f} "
    ...
    final_summary = {
        ...
        "best_metric": float(best_ckpt["best_metric"]),
        "selection_metric_name": args.selection_metric,
        "val": val_summary,
        "test": test_summary,
        "class_names": CLASS_NAMES,
    ...
    write_final_metrics_report(
        ...
        val_class_df=val_class_df,
        test_class_df=test_class_df,
        val_density_df=val_density_df,
        test_density_df=test_density_df,
    ...
    print(
        test_class_df[
            ["class_id", "class_name", "iou", "dice", "precision", "recall", "support_pixels", "pred_pixels"]
